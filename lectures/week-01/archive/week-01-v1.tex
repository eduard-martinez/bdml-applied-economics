...
\subtitle{Week 01: Fundamentos del Aprendizaje Estadístico}
...
\begin{itemize}
  \item Predecir y clasificar variables económicas con métodos de ML.
  \item Elegir, ajustar y validar modelos con desempeño \emph{out-of-sample}.
  \item Implementar todo el flujo en R (traducción opcional a Python).
  \item Leer críticamente investigación económica que usa ML.
\end{itemize}
\item \textbf{Bloques principales:}
\begin{itemize}
  \item Fundamentos del aprendizaje estadístico.
  \item Métodos supervisados: regularización, árboles y \emph{boosting}.
  \item Clasificación, ML causal y aprendizaje no supervisado.
  \item Datos no estructurados: redes neuronales, texto y LLMs.
\end{itemize}
\item \textbf{Metodología:}
\begin{itemize}
  \item Exposición teórica del profesor: intuición antes que formalismo.
  \item Aplicaciones guiadas en R con datos reales.
  \item Discusión de artículos aplicados.
\end{itemize}
\item \textbf{Evaluación:}
\begin{itemize}
  \item Taller 1: 20\%. \quad Taller 2: 20\%. \quad Proyecto final: 60\%.
\end{itemize}
\end{enumerate}
\end{frame}
...
\# & Tema \\
\hline
1 & Fundamentos del aprendizaje estadístico \quad $\leftarrow$ \textbf{hoy} \\
2 & Validación cruzada, selección de modelos y regularización \\
3 & Modelos lineales de alta dimensión: la teoría del Lasso \\
4 & Árboles, Random Forest y \emph{boosting} \\
5 & Clasificación y métricas de desempeño \\
6 & Machine learning para inferencia causal \\
7 & Aprendizaje no supervisado y reducción de dimensión \\
8 & Redes neuronales y visión por computadora \\
9 & Texto como datos y LLMs $+$ presentaciones finales \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}\itemsep3pt
  \item \textbf{Taller 1} cubre las sesiones 1--5; \textbf{Taller 2} cubre las sesiones 6--9.
  \item El \textbf{proyecto final} corre en paralelo durante todo el semestre.
  \item Cada sesión introduce un método, lo formaliza y lo aterriza en una aplicación económica con su propio conjunto de datos.
\end{itemize}
\end{frame}

%%=================================================
\begin{frame}{Talleres (2 $\times$ 20\%)}
\begin{itemize}\itemsep5pt
    \item Se realizarán \textbf{2 talleres} en parejas, para desarrollar en casa.
    \item Cada taller tiene dos componentes:
    \begin{enumerate}\itemsep3pt
        \item \textbf{Demostración matemática (40\%):} derivaciones formales de los métodos vistos. P.\,ej.: descomposición sesgo--varianza, soluciones de Ridge/Lasso vía KKT, un paso de \emph{gradient boosting} (Taller 1); PCA como problema de autovalores, un paso de retropropagación (Taller 2).
        \item \textbf{Componente aplicado (60\%):} \emph{pipeline} predictivo completo con validación cruzada sobre datos reales.
    \end{enumerate}
    \item \textbf{Entrega:} documento \texttt{.pdf} $+$ repositorio reproducible con código \texttt{.R}.
    \item Se valorará claridad, rigor y notación.
    \item Las fechas de entrega serán anunciadas con al menos \textbf{una semana} de anticipación.
\end{itemize}
\end{frame}

%%=================================================
\begin{frame}{Proyecto final aplicado (60\%)}
\small
\begin{itemize}\itemsep4pt
    \item En grupos de \textbf{2--3 estudiantes}: definir una \textbf{pregunta económica} concreta, identificar un conjunto de datos, aplicar \textbf{varios algoritmos de ML} y evaluar su desempeño \textbf{fuera de muestra}.
    \item \textbf{Entregables:}
    \begin{itemize}\itemsep3pt
        \item \textbf{Entrega 1 --- Propuesta (10\% del proyecto):} pregunta, datos, métrica objetivo y estrategia de validación.
        \item \textbf{Entrega 2 --- Reporte (60\% del proyecto):} documento de máximo \textbf{10 páginas} con resultados, comparación de modelos y diagnósticos, acompañado del repositorio reproducible.
        \item \textbf{Presentación (30\% del proyecto):} exposición en la última sesión, discutiendo la pertinencia de los métodos y sus implicaciones de política.
    \end{itemize}
    \item \textbf{Logística:} el detalle paso a paso será cargado en \textbf{Intu}.
\end{itemize}
\end{frame}
...
%%#################################################
%%#################################################
\section{Motivación: ¿por qué ML en economía?}
\begin{frame}[noframenumbering]
\tableofcontents[currentsection]
\end{frame}

%%=================================================
\begin{frame}{¿Qué entendemos por Big Data?}
\small
\begin{itemize}\itemsep5pt
  \item \textbf{Big $n$} (muchas observaciones) es solo una parte de la historia. \pause
  \item Big también es \textbf{big $p$}: muchas covariables --- a veces $p \gg n$ (registros administrativos, genómica de la firma, miles de características de una vivienda). \pause
  \item Y big también son los \textbf{datos que no surgen de fuentes tradicionales} (encuestas, cuentas nacionales):
  \begin{itemize}\itemsep3pt
    \item Datos de la web: búsquedas, precios en línea, redes sociales.
    \item Imágenes satelitales, luces nocturnas, datos geoespaciales.
    \item Texto: comunicados de banca central, noticias, sentencias judiciales.
    \item Registros transaccionales: tarjetas, telefonía móvil, GPS.
  \end{itemize} \pause
\end{itemize}

\vspace{0.2cm}
\begin{block}{Implicación}
Estos datos son masivos, de alta dimensión y sin estructura de encuesta. Los métodos clásicos de la econometría no fueron diseñados para explotarlos --- ahí entra el \emph{machine learning}.
\end{block}
\end{frame}
...
\begin{itemize}\itemsep5pt
  ...
  \end{center} \pause
  \item \textbf{¿En qué se diferencia de la econometría que ya conocen?} \pause
  \begin{itemize}\itemsep3pt
    \item Econometría clásica: estimar \textbf{parámetros} ($\beta$) con propiedades deseables e interpretarlos.
    \item ML: producir \textbf{predicciones} ($\hat{y}$) que funcionen bien en datos \textbf{nuevos}.
  \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep3pt
  ...
  \item \textbf{No son rivales:} en la sesión 6 veremos cómo la buena predicción se pone al servicio de la inferencia causal (\emph{double machine learning}).
\end{itemize}
\end{frame}

%%=================================================
\begin{frame}{El ML ya está en la caja de herramientas del economista}
\small
\textbf{Algunos ejemplos publicados:}
\begin{itemize}\itemsep5pt
  \item \textbf{Pobreza desde el cielo} (Jean et al., 2016, \emph{Science}): imágenes satelitales $+$ \emph{transfer learning} explican entre \textbf{37\% y 55\%} de la variación del consumo promedio en cinco países africanos. Clave donde no hay encuestas frecuentes. \pause
  \item \textbf{Texto de política monetaria} (Hansen, McMahon \& Prat, 2018, QJE): modelos de tópicos sobre las transcripciones del FOMC para medir deliberación y transparencia. \pause
  \item \textbf{Decisiones judiciales} (Kleinberg et al., 2018, QJE): predicción de riesgo para informar decisiones de libertad provisional. \pause
\end{itemize}

\vspace{0.2cm}
\textbf{Y en la práctica profesional colombiana:}
\begin{itemize}\itemsep3pt
  \item \emph{Scoring} de crédito en bancos y \emph{fintech}.
  \item Focalización de programas sociales (Sisbén IV).
  \item \emph{Nowcasting} de actividad económica e inflación con datos de alta frecuencia.
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep3pt
  \item $Y$: variable de respuesta (salario, impago, consumo, precio).
...
\begin{itemize}\itemsep4pt
  ...
  \item \textbf{0--1:} $L = \1\{y \neq \hat{f}(x)\}$. Para clasificación (sesión 5).
\end{itemize}
\pause
...
\textbf{¿Quién minimiza el riesgo bajo pérdida cuadrática?} Condicionemos en $X = x$ y busquemos la mejor predicción constante $c$:
\vspace{0.1cm}
...
\begin{itemize}\itemsep4pt
  ...
  \item \textbf{Irreducible} ($\sigma^2$): ni el modelo perfecto ($\hat{f} = f$) lo elimina. Es el techo de lo aprendible con estos predictores. \pause
\end{itemize}

\begin{block}{Mensaje}
Si el desempeño de un modelo parece ``malo'', la causa puede ser $\sigma^2$ alto --- no siempre hay un mejor algoritmo; a veces faltan mejores \textbf{datos}.
\end{block}
\end{frame}
...
\begin{itemize}\itemsep4pt
  ...
  \item ¿Y si la muestra es pequeña y partirla ``duele''? De eso se encarga la \textbf{validación cruzada} (sesión 2).
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep5pt
  ...
  \begin{itemize}\itemsep3pt
    \item Regularización: el parámetro de penalización $\lambda$ (sesiones 2--3).
    \item Árboles: la profundidad y el tamaño de las hojas (sesión 4).
    \item $k$-NN: el número de vecinos $k$ (sesión 5).
    \item Redes neuronales: capas, neuronas, \emph{dropout} (sesión 8).
  \end{itemize} \pause
\end{itemize}

\begin{block}{Idea clave}
Elegir el modelo es elegir \textbf{dónde pararse} sobre la curva U. Necesitamos: (i) una perilla de complejidad y (ii) un estimador honesto del error de prueba para girarla. La sesión 2 construye el segundo ingrediente.
\end{block}
\end{frame}
...
%%#################################################
%%#################################################
%% PARTE 5: LA DESCOMPOSICIÓN SESGO-VARIANZA
%%#################################################
%%#################################################
...
%%=================================================
\begin{frame}{Derivación (I): separar el error irreducible}
\small
...
%%=================================================
\begin{frame}{Derivación (II): sesgo al cuadrado más varianza}
\small
...
\vspace{0.2cm}
\textbf{¿Cuándo explota la varianza de OLS?}
\begin{itemize}\itemsep3pt
  \item Muchos predictores ($p$ grande respecto a $n$).
  \item Predictores muy correlacionados (colinealidad).
  \item Muestras pequeñas o ruidosas.
\end{itemize}
En esos casos, un estimador \textbf{levemente sesgado} pero mucho \textbf{más estable} predice mejor.
\pause

\vspace{0.2cm}
\begin{block}{Teaser de la sesión 2}
...
%%#################################################
%%#################################################
%% PARTE 6: LAS DOS CULTURAS Y EL MAPA DEL CURSO
%%#################################################
%%#################################################
\section{Las dos culturas y el mapa del curso}
\begin{frame}[noframenumbering]
\tableofcontents[currentsection]
\end{frame}
...
En la frontera, que ya se disolvió: la econometría adoptó los algoritmos (este curso) y el ML aprendió causalidad (sesión 6). Pero adoptamos la \textbf{disciplina de validación} de la cultura 2: la prueba de un modelo es predecir datos que no vio.
\end{block}
\end{frame}
...
\hline
Regresión & Sí (continua) & ¿cuánto? & precio de vivienda & 1--4 \\
Clasificación & Sí (discreta) & ¿qué clase? & ¿el hogar es pobre? & 5 \\
\emph{Clustering} & No & ¿qué grupos? & segmentar clientes & 7 \\
Reducción de dim. & No & ¿qué estructura? & índice socioeconómico & 7 \\
\hline
\end{tabular}
\end{center}
\pause
...
\begin{itemize}\itemsep4pt
  ...
  \item \textbf{No supervisado:} no hay $y$; buscamos estructura en los $x$ (grupos, factores latentes, dimensiones).
  \item Los datos no estructurados (texto, imágenes; sesiones 8--9) combinan ambos mundos: primero se \emph{representan} (no supervisado), luego se predice con ellos (supervisado).
\end{itemize}
\end{frame}

%%=================================================
\begin{frame}{Todo el curso en una diapositiva}
\small
Cada sesión responde, con una herramienta distinta, la misma pregunta: \textbf{¿cómo controlar la complejidad para generalizar?}
\pause
\begin{itemize}\itemsep5pt
  \item \textbf{Validación cruzada (S2):} estima honestamente el error de prueba para girar las perillas. \pause
  \item \textbf{Regularización (S2--S3):} compra reducción de varianza pagando con sesgo ($\lambda$). \pause
  \item \textbf{Ensambles (S4):} promedian muchos modelos para bajar la varianza (Random Forest) o suman aprendices débiles para bajar el sesgo (\emph{boosting}). \pause
  \item \textbf{Métricas de clasificación (S5):} eligen la pérdida correcta cuando los errores tienen costos asimétricos. \pause
  \item \textbf{ML causal (S6):} usa la buena predicción al servicio de un $\beta$ creíble. \pause
  \item \textbf{No supervisado (S7):} comprime la dimensión sin necesidad de $y$. \pause
  \item \textbf{Redes, texto y LLMs (S8--S9):} clases $\mathcal{F}$ ultra flexibles $+$ regularización agresiva $+$ muchos datos. \pause
\end{itemize}

\begin{block}{Mensaje}
Si entienden la sesión de hoy --- riesgo, sobreajuste, sesgo--varianza --- entienden \emph{por qué existe} cada método del resto del curso.
\end{block}
\end{frame}
...
\begin{enumerate}\itemsep8pt
  \item \textbf{Predicción $\neq$ inferencia causal:} $\hat{y}$-problems y $\beta$-problems son preguntas distintas con herramientas distintas --- y muchos problemas de política son de predicción.
  \item El objetivo del ML supervisado es minimizar el \textbf{riesgo de generalización} $R(f) = \E[L(Y, f(X))]$, no el error en la muestra.
  \item El \textbf{error de entrenamiento es optimista}: evaluar donde se entrenó premia la memorización. Separar entrenamiento y prueba es la regla de oro.
  \item \textbf{Descomposición fundamental:} $\MSE = \text{Sesgo}^2 + \text{Varianza} + \sigma^2$. La complejidad baja el sesgo y sube la varianza: el óptimo es interior.
  \item Aceptar \textbf{algo de sesgo} para reducir mucha varianza mejora la predicción --- la idea que motiva la regularización (próxima sesión).
\end{enumerate}
\end{frame}
...
\begin{itemize}\itemsep4pt
  \item Mullainathan \& Spiess (2017), \emph{Machine Learning: An Applied Econometric Approach}, JEP. \\ \textcolor{gray}{Guía: concéntrense en la distinción $\hat{y}$ vs.\ $\hat{\beta}$ y en la Tabla 1.}
  \item Varian (2014), \emph{Big Data: New Tricks for Econometrics}, JEP. \\ \textcolor{gray}{Guía: secciones de entrenamiento/prueba y árboles de decisión.}
\end{itemize}

\textbf{Lecturas recomendadas:} Breiman (2001, \emph{Two Cultures}); Kleinberg et al.\ (2015, \emph{Prediction Policy Problems}).
\pause
...
\vspace{0.2cm}
\textbf{Próxima sesión:} validación cruzada, bootstrap, selección de modelos y regularización (Ridge, Lasso, Elastic Net) --- y la primera aplicación guiada en R.
\end{frame}
...
\footnotesize
\begin{itemize}\itemsep2pt
  \item James, G., Witten, D., Hastie, T., \& Tibshirani, R. (2021). \emph{An Introduction to Statistical Learning} (2.\ ed.). Springer. Cap.\ 2. [ISLR]
  \item Hastie, T., Tibshirani, R., \& Friedman, J. (2009). \emph{The Elements of Statistical Learning} (2.\ ed.). Springer. Caps.\ 2 y 7. [ESL]
  ...
  \item Jean, N., et al.\ (2016). Combining Satellite Imagery and Machine Learning to Predict Poverty. \emph{Science}, 353(6301), 790--794.
\end{itemize}
\normalsize
\end{frame}
